import { Check, Edit, Trash2 } from 'lucide-react'; // Usamos 'Check' en lugar de 'CheckCircle'
...
const QuestionCard = ({ question, onEdit, onDelete }) => {
  return (
    <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 flex flex-col md:flex-row gap-6 hover:shadow-lg transition-shadow">
      ...
      {/* Sección Izquierda: Pregunta y Respuestas */}
      ...
      {/* Sección Derecha: Metadatos y Acciones */}
      <div className="flex-shrink-0 w-full md:w-52 flex flex-row-reverse md:flex-col justify-between items-center md:items-end border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-6 mt-4 md:mt-0">
        ...
        <div className="flex gap-4">
          <button onClick={onEdit} className="p-2 rounded-full text-gray-500 hover:bg-blue-100 hover:text-brand-blue transition-colors"><Edit size={24} /></button>
          <button onClick={onDelete} className="p-2 rounded-full text-gray-500 hover:bg-red-100 hover:text-brand-red transition-colors"><Trash2 size={24} /></button>
        </div>
      </div>
    </div>
  );
